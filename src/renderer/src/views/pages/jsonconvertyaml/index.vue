...
<script setup name="JsonConvertYaml">
...
const convertType = ref('jsonToYaml')
...
watch(convertType, () => {
    ...
    // 切换语言
    if (convertType.value === 'jsonToYaml') {
        originalEditorInstance.setLanguage('json')
        modifieEditorInstance.setLanguage('yaml')
    } else if (convertType.value === 'yamlToJson') {
        originalEditorInstance.setLanguage('yaml')
...
const handleTypeConvert = () => {
    ...
    try {
        if (convertType.value === 'jsonToYaml') {
            ...
            const convertResult = YamlConverter.jsonToYaml(originalEditorVal);
            modifieEditorInstance.setEditorValue(convertResult)
        } else if (convertType.value === 'yamlToJson') {
            const originalEditorVal = originalEditorInstance.getEditorValue();
            const convertResult = YamlConverter.yamlToJson(originalEditorVal);
            modifieEditorInstance.setEditorValue(formatJson(convertResult))
        }
    } catch (error) {
        Message({
...
const handleBeautifyCode = () => {
    ...
    if (originalEditorVal) {
        if (convertType.value === 'jsonToYaml') {
            ...
            originalEditorInstance.setEditorValue(YamlConverter.formatYaml(originalEditorVal))
        }
    }
}
...
const handleCompressCode = () => {
    ...
    if (originalEditorVal) {
        if (convertType.value === 'jsonToYaml') {
            ...
            originalEditorInstance.setEditorValue(YamlConverter.compressYaml(originalEditorVal))
        }
    }
}
