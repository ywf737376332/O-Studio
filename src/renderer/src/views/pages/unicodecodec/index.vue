    ...
    <div class="split-panes">
      <splitpanes horizontal class="default-theme" style="height: 100%">
        <pane min-size="20" size="45">
          <TextArea v-model="data.textSourse" :maxlength="100000" :placeholder="`输入需要转换的文本`"></TextArea>
        </pane>
...
<script setup name="UnicodeCodec">
...
import UnicodeConverter from '@/utils/UnicodeConverter'

const convertType = ref('cnToUnicode')
const data = reactive({
  textSourse: "",
  textTarget: ""
})
...
watch(data, (value) => {
  if (convertType.value === "cnToUnicode") {
    data.textTarget = UnicodeConverter.chineseToUnicode(value.textSourse)
  } else if (convertType.value === "unicodeToCn") {
    data.textTarget = UnicodeConverter.unicodeToChinese(value.textSourse)
  }
})
